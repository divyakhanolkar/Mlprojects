...
# Define session state to store generated blog and feedback
...
if st.button("Generate Blog"):
    if not topic:
        ...
        with st.spinner("Generating your blog... ⏳"):
            ...
            st.session_state.generated_blog = result["final_blog"]  # Save generated blog
            st.success("✅ Blog Generated!")
            st.subheader("📌 Generated Blog:")
            st.write(st.session_state.generated_blog)

        # **Feedback section will appear after the blog is generated**
        st.subheader("💬 Provide Feedback:")
        feedback = st.text_area("Provide feedback on the blog (optional):", value=st.session_state.feedback)

        if feedback:
            st.session_state.feedback = feedback  # Save feedback

            # **Automatic Blog Update on Feedback Entry**
            with st.spinner("Incorporating your feedback... ⏳"):
                result_with_feedback = app.invoke({"topic": topic, "feedback": st.session_state.feedback, "final_blog": st.session_state.generated_blog})
                st.session_state.generated_blog = result_with_feedback["final_blog"]  # Save updated blog
                st.success("✅ Blog Updated with Your Feedback!")
                st.subheader("📌 Updated Blog:")
                st.write(st.session_state.generated_blog)  # Display the updated blog
